  ...
  return function markdown_taxonomy(files, metalsmith, done) {
    ...
    const process = (file, key, cb) => {
      ...
      if (minimatch(key, pattern)) {
        ...
        tree[key] = tokens.reduce(({ taxonomy, current, heading, item, dictionary }, {type, depth, text}) => {
          const [t, c, h, i, d ] =
            (type === "heading" && depth === 1 ) ? [ { ...taxonomy, [current_name]: {} }, current_name, '', false, dictionary]
          : (type === "heading" && depth === 2 ) ? [ { ...taxonomy, [current]: { ...taxonomy[current], [text]: [] } }, current, text, false, dictionary]
          : (type === "list_item_start" ) ? [ taxonomy, current, heading, true, dictionary]
          : (type === "text" && item ) ? [ { ...taxonomy, [current]: { ...taxonomy[current], [heading]: [ ...taxonomy[current][heading], transclusionLink(text) ] } }, current, heading, true, { ...dictionary, [transclusionLink(text)]: { [current]: [ ...dictionary[transclusionLink(text)] ? dictionary[transclusionLink(text)][current] : [] , heading ] }}]
          : (type === "list_item_end" ) ? [ taxonomy, current, heading, false, dictionary ]
    ...
    async.mapValues(files, process, (err) => {
      ...
      const dictionary = Object.keys(tree).reduce(function(acc, val) {
        const merge = Object.keys(tree[val].dictionary).reduce(function(a, v) {
      ...
      // For now transform transclusion into link and add /index.md
      const mapping = Object.keys(dictionary).reduce(function(acc, val) {
        const link = val + '/index.md'
        return { ...acc, [link]: dictionary[val]}
      }, {})

      debug('mappings: ', Object.keys(mapping).length);

      Object.keys(files).forEach(key => {
        const match = Object.keys(mapping).includes(key)
        if (match) {

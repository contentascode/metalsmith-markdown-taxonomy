...
function plugin(options) {
  var _ref = options || {},
      ...
      permalinks = _ref$permalinks === undefined ? true : _ref$permalinks;
  ...
  return function markdown_taxonomy(files, metalsmith, done) {
    ...
    var process = function process(file, key, cb) {
      ...
      if (minimatch(key, pattern)) {
        ...
        tree[key] = tokens.reduce(function (_ref2, _ref3) {
          ...
          var _ref4 = type === "heading" && depth === 1 ? [Object.assign({}, taxonomy, { [current_name]: {} }), current_name, '', false, dictionary] : type === "heading" && depth === 2 ? [Object.assign({}, taxonomy, { [current]: Object.assign({}, taxonomy[current], { [text]: [] }) }), current, text, false, dictionary] : type === "list_item_start" ? [taxonomy, current, heading, true, dictionary] : type === "text" && item ? [Object.assign({}, taxonomy, { [current]: Object.assign({}, taxonomy[current], { [heading]: [].concat(_toConsumableArray(taxonomy[current][heading]), [transclusionLink(text)]) }) }), current, heading, true, Object.assign({}, dictionary, { [transclusionLink(text)]: { [current]: [].concat(_toConsumableArray(dictionary[transclusionLink(text)] ? dictionary[transclusionLink(text)][current] : []), [heading]) } })] : type === "list_item_end" ? [taxonomy, current, heading, false, dictionary] : [taxonomy, current, heading, item, dictionary],
    ...
    async.mapValues(files, process, function (err) {
      ...
      var mapping = Object.keys(dictionary).reduce(function (acc, val) {
        var link = val + '/index.md';
        return Object.assign({}, acc, { [link]: dictionary[val] });
      }, {});

      // debug('mapping', JSON.stringify(mapping, true, 2));


      Object.keys(files).forEach(function (key) {
        var match = Object.keys(mapping).includes(key);
        if (match) {
